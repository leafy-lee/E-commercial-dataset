...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler):
    ...
    for idx, (name, samples, targets) in enumerate(data_loader):
        ...
        '''
        # print(saliency_loss , attn_loss, attn_loss.shape,"here comes the bugs")
        ...
        loss = saliency_loss*2 + attn_loss
        attn_loss_meter.update(attn_loss, targets.size(0))
        saliency_loss_meter.update(saliency_loss, targets.size(0))
        optimizer.zero_grad()
    ...
    for i, maps in enumerate(test_map):
        name_i = test_name[i].split("/")[-1]
        path = test_loc + str(epoch)+"_epoch_" + name_i
...
def validate(config, data_loader, model):
    # waiting
    criterion = Maploss()
    saliency_loc = config.OUTPUT + '/ans/new_saliency/'
    if not os.path.exists(saliency_loc):
        os.makedirs(saliency_loc)
    save_loc = config.OUTPUT + '/ans/new_ocr/'
    ...
    with torch.no_grad():
        for idx, (idx_name, images, target, ocr_target) in enumerate(data_loader):
            ...
            for i, ocr_out in enumerate(ocr_outs):
                ...
                image=cv2.imread("/mnt/disk4T/lyf/ECdata/ALLSTIMULI/"+str(idx_name[i]) + '.jpg')
    ...
    print("average kl, cc is %f, %f"%(klall/cnt, ccall/cnt))
